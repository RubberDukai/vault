...
| 1.10 | Language learning: English + Japanese first, then Spanish, Mandarin, Hindi | **PARTIAL** | 1,322 cards with spaced repetition; written grammar guides for all four; cards spoken aloud with the Say it button (P) or Auto, using the voices installed in Windows — offline, but only for languages whose voice pack is installed (Settings › Time & language › Speech). Still to do: the rest of N5 and A1 |
