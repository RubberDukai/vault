...
**Every feature asked for is built.** Nothing on this list is open or in progress.

Three things stand between here and a first release, and only the first is real work:

1. **The Wikipedia download finishing** — 23 of 49 GB at the time of writing. It needs nothing but time and the vault window left open; closing that window stops it, and it resumes where it left off.
2. **`node tools/make-installer.js`** — builds **VaultSetup.exe** (34 MB) and a portable zip. Done and tested; rebuild it whenever the program changes.
3. **Installing it on the other devices**, either with that setup file or, for a machine that has never been online, with `node tools/make-portable.js <folder> --with-node --with-packs --with-maps`.

Two things are deliberately left undone, both from the agents' audit rather than from a request, and neither would hold up a release:

- The **first search after launch** waits a moment while the indexes build in the background.
- The **map drawing tools on a phone** sit below the map rather than over it, so you scroll to reach them. The map itself works properly at phone width.
...
| 11.4 | Audit findings (agents' first pass, 20 Sep) | **DONE** | Every item fixed and re-verified; see 11.6 for the second pass. Range-crash, reflected XSS on a missing pack page, body size limits, an isolated browser profile, sharing off by default, uncaught-exception handlers, an offline basemap shipped, decompression caps, and fsync-before-rename so a power cut leaves the old file or the new one and never half of either |
...
| 11.6 | Agents' review v2 (7 dimensions) — punch list | **DONE** | Everything Critical, High and Medium is fixed — see the git history from a0af122 for the item-by-item record: the drive-by file write through /api/downloads, cross-site requests, a Content-Security-Policy on pack and book pages, prototype pollution, the ZIM abort, the one startup ping, search ranking the handbook properly, PDFs on phones, corrupt-state quarantine, the charcoal and generator warnings, the leaked username, CSV formula injection and the amber contrast. Finding 15 (tests) became 11.8; finding 17 (profile wipe and encryption) became 11.10 and 11.11. Left undone by choice: the cold-start first search, and the map drawing tools as a phone overlay |
| 11.7 | Education to a teachable KS1–3 | **DONE** | Approved 22 Sep; scope settled 24 Sep (**English as a second language dropped** at your request, in favour of English KS1/KS2 and reading). **Every lesson in the road map is now written — 46 lessons across five subjects, KS1 to KS3.** Teaching guide: how to run a school at home, the daily rhythm, what "ready for the next stage" looks like, and the full road map. Lessons grouped by key stage on the School page; every Answers section folded until revealed. **Maths (16)** KS1 six-lesson sequence; KS2 arithmetic, fractions, decimals and percentages, area/angles/shape, graphs and records; KS3 algebra, chance and data, equations and formulae, geometry with proof, ratio and rates. **English (11)** KS1 reading, handwriting and spelling, sentences; KS2 writing that lasts, grammar and punctuation, reading for meaning, poetry; KS3 thinking clearly, the essay, the report and the letter, speaking and debate. **Science (12)** KS1 living things, materials, seasons; KS2 finding things out, living things, earth and weather, forces/light/sound, the human body; KS3 energy, matter and reactions, cells/genes/evolution, the Earth in space. **The World (6)** reading a map, Britain through time, the ancient world, continents and climate, the modern world, how societies are run. Every lesson has exercises and a folded answer key; all 75 encyclopedia links verified against the installed pack. **German** added (guide + five decks, 375 cards). **Reading shelves**: 61 Project Gutenberg books graded KS1/KS2/KS3, installed. Still open: listening comprehension (audio); more languages if wanted |
...
| 11.3 | Finish every requested feature and partial before rollout | **DONE (25 Sep)** | Every feature asked for in this project is built and verified in the running app. The one row still marked partial is 1.7 (a packaged installer), which is deliberate: the window is Chrome or Edge in app mode, and bundling a browser would mean Electron and 200 MB for no gain — the portable copy with Node bundled does the job instead. Remaining before a first release: the Wikipedia download finishing, then make-portable, then installing on the other devices |
| 11.8 | A test suite (audit finding 15) | **DONE** | 96 tests, Node's own runner, no dependencies, offline, about a second and a half: `npm test`. **server.test.js** starts a real server on an ephemeral port with an empty data directory and re-checks every security fix from the review — the vault header on state-changing calls, the cross-site refusal, the Host allow-list, Kiwix-only download hosts, filenames reduced to a plain .zim, `__proto__` never reaching an object key, and a malformed Range or body leaving the process alive. **content.test.js** checks the content like code: the frontmatter every lesson needs, no clashing orders, every set of exercises having answers, well-formed `wiki:` links, escaped pipes in table cells, a valid pack catalogue, no repeated flashcards. **store.test.js**, **packs.test.js**, **srs.test.js**, **markdown.test.js**, **sheets.test.js** cover durability, the download queue and its lock, the review scheduler, the renderer and CSV. Writing them found two real faults, both fixed: frontmatter turned `order: 1.5` into a string because only whole numbers were parsed, and seven early lessons set exercises with no answer key — those now have one (Learning to read has no exercises by design) |
| 11.9 | Listening comprehension for the languages | **DONE** | The last open item from the audit's language verdict. **17 spoken passages** across the five languages — Spanish 4, German 4, Japanese 3, Mandarin 3, Hindi 3 — graded A1/N5/HSK1 up to A2/HSK2, on the market, asking the way, the doctor and the garden, each with a transcript, a line-by-line English translation, a vocabulary table and five or six comprehension questions with folded answers. Japanese, Mandarin and Hindi carry a reading line (rōmaji, pinyin, transliteration) as well as the script. **The player** speaks the passage with the same offline Windows voice the flashcards use — nothing streamed, no audio files shipped: play the whole passage or one line, three speeds, the speaking line highlighted, and the transcript hidden until the listener has had a fair go without it. Wrong answers say "listen again" rather than giving it away. **The voice panel** on the Languages page now says which of the five this machine can actually speak and gives the exact Windows path to add the rest — on this machine none of the five are installed, so every passage currently falls back to showing its transcript, which is the designed behaviour and not a fault. A test checks every passage has translations on every line, speakers that exist, and answers that point at real options |
| 11.10 | Wipe the vault window's browsing history (audit finding 17a) | **DONE** | On by default, switchable in Setup → Privacy. The window has its own browser profile inside the vault folder; its history, cache and session are wiped on the way out and again at startup in case the last run ended in a power cut. Local Storage survives, because that is where the theme, the phosphor colour, the selected person and the map view live and wiping them would mean choosing your colours again every launch. The exit wipe is best-effort — a hard kill or a power cut runs nothing — so the startup wipe is the guarantee, and that is the path the tests cover |
| 11.11 | A PIN, and encryption at rest (audit finding 17b) | **DONE** | Optional, off by default, Setup → Privacy. Notes, messages, the calendar, map markings, profiles and progress are encrypted with AES-256-GCM; the library is deliberately left alone, being public knowledge that would only be slower to write. A random data key is wrapped twice — once under the PIN, once under a twelve-word recovery phrase — so changing the PIN rewrites one small blob instead of re-encrypting everything, and a forgotten PIN is survivable if the phrase was written down. The wordlist is 256 words with no prefixes or homophones, giving 96 bits; the phrase is shown once. scrypt is tuned so a guess costs about half a second, and the page says plainly that six digits is not a passphrase. The lock screen draws nothing of the vault behind it. Two bugs found in testing: the launcher crashed on a locked vault before the server could start, and the Setup page redrew over the one-and-only showing of the recovery phrase |
| 11.12 | Where you are, for anywhere on Earth | **DONE** | The night sky, almanac and calendar were reckoning from wherever the map was last scrolled, falling back to the middle of Britain, and nothing said so. Setup → Where you are takes a town from the installed maps, plain coordinates, or the map centre, and keeps it on the server so every device agrees and a profile wipe does not lose it. Verified against Wellington: the sky page correctly reports Polaris as not visible south of the equator, and says the Cross replaces the Plough |
| 11.13 | Search: the encyclopedia first, places, and did-you-mean | **DONE** | "Bronze age" opened with AgenDAV from the Arch Linux wiki — every pack contributed its first eight titles in pack order, so the right answer was below the fold. Hits are now scored and ranked together (exact beats prefix beats mention, shorter titles win, disambiguation pages sink) with a general encyclopedia weighted above a niche wiki as a tilt only, so "pacman" still finds ArchWiki. A clear article match is lifted out and shown on its own. Place names from the map appear in the main box and open the map with a marker. Misspellings are corrected against words the vault actually uses often — with 70,000 book sections indexed, mere presence proves nothing |
| 11.14 | Maps: speed, road names, label hierarchy, and the rest of the world | **DONE** | Panning central London took 83 ms a frame because every frame redrew every path; tiles are now drawn once and copied, and labels are left off while the map moves, giving 2.9 ms dragging. Road names were in the data and never drawn — now written along the road, on its longest straight run, never upside down. Place names were all one size in tile order so a hamlet could take a city's space; they are now ranked by the importance the data carries and drawn biggest-first with a dot at the settlement. Nine more offline regions added (the Americas, Africa, the Middle East, South, South East and East Asia, Australasia, northern Asia), and the one online entry — a 45-byte pointer file — is grouped separately and labelled |
| 11.15 | Games, and the chess bug | **DONE** | Chess was unplayable: the board asks both games whether a piece is mid-multi-jump, draughts answered null, chess answered undefined, and undefined !== null, so every piece was locked. Added Othello, Klondike patience, blackjack and roulette — the last two with counters rather than money, and both saying plainly why the house wins. 35 tests on the rules, including twenty thousand spins checked against the 97% a single-zero wheel really returns |
| 11.16 | Links and the encyclopedia | **DONE** | An audit of all 94 lessons and chapters found named topics with an article sitting in the encyclopedia and no link to it: 47 added. Two bugs behind it — the renderer escaped a link target twice, so every link with an apostrophe was dead; and 31 foraging links pointed at Latin names that are in the full Wikipedia but not the Simple English one installed meanwhile. A link whose article is not in the installed pack now goes to a search for the topic instead of a missing page. Verified: 220 links across 94 pages, none dead |
| 11.17 | A setup file people can download and run | **DONE (25 Sep)** | `node tools/make-installer.js` builds **VaultSetup.exe**, 34 MB: double-click, it installs to the user's own folder with no administrator and no UAC prompt, makes a Desktop and Start Menu shortcut, and registers an uninstaller that asks before touching anything they made. Running it again updates the program and keeps their notes, settings and packs. It carries the program, handbook, lessons, languages and a copy of Node.js — **no encyclopedias, maps or books**: those are chosen inside the Vault on the Setup page, which already offers 138 of them. A portable .zip is built alongside for machines that refuse unsigned executables. Built with makecab and iexpress, which ship with Windows, so the toolchain needs no downloads either. Expect a SmartScreen warning until somebody pays for a code-signing certificate |
